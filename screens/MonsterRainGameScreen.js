...
const MonsterRainGameScreen = ({ route }) => {
  ...
  const handleMonsterPress = useCallback(
    (id, x, y) => {
      if (gameOver) return;
  ...
  useEffect(() => {
    const moveMonsters = () => {
      setMonsters((prev) => {
        ...
        const reachedBottom = updatedMonsters.filter(
          (monster) => monster.y >= SCREEN_HEIGHT
        );
  ...
  const renderMonster = useCallback(
    (monster, index) => {
      ...
      return (
        ...
        >
          <Animated.View
            style={{ transform: [{ scale: scaleAnims[monster.id] }] }}
          >
            <Image
              ...
              resizeMode="contain"
            />
            <Text style={styles.monsterName}>{t(monsterData.name)}</Text>
          </Animated.View>
        </TouchableOpacity>
      );
    },
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.hudContainer}>
          <View style={styles.hudItem}>
            <Text style={styles.hudLabel}>{t("Score")}</Text>
          ...
          <View style={styles.hudItem}>
            <Text style={styles.hudLabel}>{t("Lives")}</Text>
